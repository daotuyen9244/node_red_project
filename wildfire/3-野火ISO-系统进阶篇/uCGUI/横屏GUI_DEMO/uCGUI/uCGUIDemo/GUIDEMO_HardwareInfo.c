...
void GUIDEMO_HardwareInfo(void) {
  #if   (LCD_CONTROLLER == 13705) | (LCD_CONTROLLER == 1386) | (LCD_CONTROLLER == 13806)
    ...
    GUIDEMO_SetColor(GUI_WHITE);
      ...
      _Show("LUT",           "256 index colors");
			#elif (LCD_CONTROLLER == 8875) | (LCD_CONTROLLER == 8875)
      #if GUIDEMO_LARGE
        GUI_DispStringAt("RAIO RA8875\n", 10, 65 );
      #else
        GUI_DispStringHCenterAt("RAIO RA8875\n", LCD_XSIZE >> 1, LCD_YSIZE / 14);
      #endif
      _Show("On chip VRAM",  "768 kb");
      _Show("Max. colors",   "65536 (up to 16 bpp)");
      _Show("Display types", "Mono / color / TFT (analog / digital)");
      _Show("LUT",           "256 index colors");
    #endif
