...
char path[100]="0:";						//扫描SD卡的初始目录，扫描是递归函数，递归越深，此数组取值也要越大(由于RAM限制 此处取少点)	
...
/**
  ...
  * @retval none
  */
static void mp3_AutoPlayer(const char *filename)
{
		...
		{		
			...
			switch(player_state)
			{
				case S_PLAY:							//播放状态	
					...
					//打开playlist，读取音频文件名
					fres = f_open (&file, "0:mp3player/playlist.txt", FA_READ);
					fres = f_lseek (&file, play_index*FILE_NAME_LEN);//play_index
					fres = f_read(&file, music_name, FILE_NAME_LEN, &rw_num);
					fres = f_close (&file);
