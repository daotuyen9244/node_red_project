...
void LCD_GetLine_BGR24(uint16_t x , uint16_t y,uint16_t width,uint8_t * pdata)
{
	uint16_t i;
    volatile uint8_t tmp;
    
	//LCD_SetCursor(x, y);
	LCD_OpenWindow(x, y, width, 1);		//开窗
	...
	for(i=0;i<width;i++)
	{
        tmp             =   *(__IO uint16_t *)Bank1_LCD_D;      /*FIRST READ OUT DUMMY DATA */
        //BGR
		*(pdata + 2)    =   *(__IO uint16_t *)Bank1_LCD_D;      /*READ OUT RED DATA         */
        *(pdata + 0)    =   *(__IO uint16_t *)Bank1_LCD_D;      /*READ OUT BLACK DATA       */
        *(pdata + 1)    =   *(__IO uint16_t *)Bank1_LCD_D;      /*READ OUT GREEN DATA       */
        pdata           +=  3;                                  /*切换到 下一个像素         */
	}
}
/*-------------------------------------------------------------------------------------------------------*/
